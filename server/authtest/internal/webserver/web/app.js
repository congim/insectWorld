...
function showResult(elemId, data, successKey) {
    const elem = document.getElementById(elemId);
    if (data.success || (successKey && data[successKey])) {
...
document.getElementById('btn-sut-status').addEventListener('click', async function() {
    ...
    if (data.success) {
        ...
        elem.innerHTML = '<span class="status-success">状态: ' + (statusMap[s.status] || '未知') +
            '</span> | PID: ' + s.pid + ' | 退出码: ' + s.exit_code;
...
async function sendAuth(type, fields) {
    ...
    if (data.success) {
        ...
        if (r.response && r.response.success) {
            elem.innerHTML = '<span class="status-success">成功</span> | 耗时: ' + r.duration_ms + 'ms\n' + formatJSON(r.response);
        } else {
            elem.innerHTML = '<span class="status-failure">失败</span> | 耗时: ' + (r.duration_ms || 0) + 'ms\n' + formatJSON(r.response || r);
        }
        addHistory(type, body, r.response || r, r.duration_ms);
        return r.response;
    } else {
...
document.getElementById('btn-e2e-run').addEventListener('click', async function() {
    ...
    if (data.success) {
        ...
        const statusCls = report.overall_passed ? 'status-success' : 'status-failure';
        let html = '<span class="' + statusCls + '">通过率: ' + report.pass_rate + '% | ' +
            (report.overall_passed ? '全部通过' : '存在失败') + '</span>\n';
        ...
        for (const step of report.steps) {
            ...
            html += '<tr><td>' + step.step_name + '</td><td class="' + cls + '">' +
                (statusMap[step.status] || '未知') + '</td><td>' + step.duration_ms + '</td><td>' +
...
document.getElementById('btn-e2e-failure').addEventListener('click', async function() {
    ...
    if (data.success) {
        ...
        for (const r of results) {
            ...
            html += '<tr><td>' + r.step_name + '</td><td class="' + cls + '">' +
