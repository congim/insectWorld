...
const (
	...
	ErrCodeBuildingNotFound      = 19011 // 建筑实例不存在
	ErrCodeTrainingNotFound      = 19012 // 训练任务不存在
	ErrCodeRepositoryUnavailable = 19013 // Growth持久化暂不可用
)
...
var (
	...
	ErrBuildingNotFound      = &GameError{Code: ErrCodeBuildingNotFound, Msg: "建筑不存在"}
	ErrTrainingNotFound      = &GameError{Code: ErrCodeTrainingNotFound, Msg: "训练任务不存在"}
	ErrRepositoryUnavailable = &GameError{Code: ErrCodeRepositoryUnavailable, Msg: "成长数据暂不可用"}
)
